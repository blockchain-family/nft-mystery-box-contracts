...
async function main() {
    ...
    logger.log(`Giver balance: ${locklift.utils.convertCrystal(await locklift.ton.getBalance(locklift.networkConfig.giver.address), 'ever')}`);

    keyPairs = await locklift.keys.getKeyPairs();

        // deploy account
    const Account10 = await locklift.factory.getAccount("Wallet");
    ...
    Account10.setKeyPair(keyPairs[0]);

    migration.store(Account10, 'Account10');
    Account10.afterRun = afterRun;

    logger.success(`Owner: ${Account10.address}`);
    ...
    // logger.log(`Account10 WEVER balance: ${balance}`);balance
